...
function db_ui_option($guid, $tabla, $texto, $valor, $descripcion, $where="1")
{
   $c = "SELECT $valor, $texto, $descripcion FROM $tabla WHERE $where";
   $r = db_consultar($c);
   $html = '';
   while ($row = mysql_fetch_assoc($r)) {
       $html .= '<p><strong>' . ui_input($guid, $row[$valor], "radio","","") . $row[$texto] . '</strong></p><div><p>'. $row[$descripcion] . '</p></div>';
   }
   return $html;
   }
